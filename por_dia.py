...
# ============================================
# FRASES GENÉRICAS PARA TEMAS NO PREDEFINIDOS
# ============================================
VERBOS_GENERICOS = [
...
def dividir_en_parrafos(tema, num_parrafos):
    ...
    num_intermedios = num_parrafos - 3
    if num_intermedios < 1:
        num_intermedios = 1
    
    if len(frases_tema) >= num_intermedios:
        seleccionadas = random.sample(frases_tema, num_intermedios)
    else:
        seleccionadas = frases_tema.copy()
...
def crear_video(tema, dia_semana, numero):
    ...
    for i, parrafo in enumerate(parrafos):
        ...
        draw.text((x_firma, y_firma), firma, font=font_firma, fill='white', stroke_width=3, stroke_fill='black')
